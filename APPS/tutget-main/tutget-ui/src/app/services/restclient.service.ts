...
export class RestclientService {
  ...
  BASE_URL: string = 'https://localhost:8443/api'
  ...
  async getrawjson(url: string, external: boolean): Promise<any>{
    ...
    return await firstValueFrom(this.http.get(finalURL, this.httpOptions))
      // .catch((err) => {
      //   console.log(err);
  ...
  async postjsonReturnString(url: string, rawdata: any): Promise<any>{
    ...
    return await firstValueFrom(this.http.post(this.BASE_URL + url, data, { headers, responseType: 'text', withCredentials: true }))
      // .catch((err) => {
      //   console.log(err);
      //   this.router.navigate(['/error'], err);
      // });
  ...
  async delete(url: string): Promise<any>{
    return await firstValueFrom(this.http.delete(this.BASE_URL + url, { withCredentials: true }))
      // .catch((err) => {
      //   console.log(err);
      //   this.router.navigate(['/error'], err);
      // });
